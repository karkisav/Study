...
def load_data(data_dir):
    ...
    for category in os.listdir(data_dir):
        """Use os.path.join to go into the subfolders"""
        category_path = os.listdir(os.path.join(data_dir, category))
        for image in category_path:
            image_path = os.path.join(data_dir, category, image)
            #resizing images so that they are uniform
            resized_image = cv2.resize(cv2.imread(image_path), (IMG_HEIGHT, IMG_WIDTH))

            images.append(resized_image / 255.0)
            labels.append(int(category))
        print(f"Data Loaded for the category: {category}")
